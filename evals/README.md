...
- **Fix PRs prove their fix in the PR.** A feature PR whose body carries a line-anchored `Evals: <space-separated case ids>` marker (or `Evals: smoke`; optional `Eval-Epochs: N`, default 6) runs exactly those cases on the PR head — gated and pass^k (the runner's `-passk` flag): every targeted case must pass every epoch before merge, because a fix that passes 4 of 6 epochs is not a demonstrated fix. The `evals` required check turns red and blocks the merge otherwise; each run posts its report as a PR comment. PRs without the marker fall back by content: a PR touching `skills/mentor/**` (catalog and skill edits — the product surface) automatically runs the smoke tier at 3 epochs with strict-majority gating — release-gate semantics, not pass^k, because the question is "did this content PR break the product?", not "does a declared fix hold every epoch" — so known single-epoch flakes are absorbed rather than blocking; the rate tier stays release-only, so a majority-fail here always gates. Unmarked PRs touching nothing under `skills/mentor/` keep the skipped-run behavior. Declare `Evals: smoke` (or add cases to the list) when a change could ripple beyond its target; the full 3-epoch suite still gates every release on `release/next`, so a missed side effect is caught where it always was — minus the blind merge.
