...
| Different aspects need different model strengths | Multi-provider | Use a reasoning model for architecture, a fast model for summarization |
...
### 5. Multi-Provider — Different models for different angles
**Level:** Advanced | **Tools:** OpenCode

Complex research benefits from multiple perspectives. OpenCode's multi-provider support lets you route different aspects of the investigation to different models: use a reasoning model for architecture analysis, a fast model for summarizing documentation, and a different provider for a second opinion on the final recommendation. This reduces single-model blind spots.

**Try it now:**
> Use the reasoning model to analyze the architectural tradeoffs between event sourcing and traditional CRUD for our order management system. Then use the fast model to summarize the top 5 GitHub issues for each major event sourcing library in our language. Finally, get a second opinion from a different provider on whether event sourcing is appropriate for our scale (~10K orders/day, 3-person team).

**Why this works:** Every model has blind spots shaped by its training data and reasoning patterns. Using multiple models for different research tasks is like getting multiple expert opinions — you're more likely to catch something that a single model would miss or gloss over.

**Pros:**
- Reduces single-model bias in research conclusions
- Can match model strengths to task types (reasoning vs. summarization)
- Second opinions catch overconfident or hallucinated claims

**Cons:**
- Only available in OpenCode, not Claude Code
- Higher complexity in managing multiple model outputs
- Diminishing returns for straightforward research questions

**Deeper:** See `approaches/multi-provider.md`
