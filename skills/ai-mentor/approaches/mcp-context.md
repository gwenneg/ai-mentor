...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | `.mcp.json` config at project and user scope, stdio, HTTP (recommended for remote), SSE (deprecated), WebSocket transports, MCP Tool Search |
| OpenCode | Native | Full MCP client support with local and remote servers, OAuth |
| Cursor | Partial | Full MCP support on all plans, .cursor/mcp.json config, stdio/SSE/HTTP transports |
| aider | Partial | MCP client support via --mcp-server flag, evolving |
