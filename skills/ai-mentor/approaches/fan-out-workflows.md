...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | Built-in workflow runtime with `agent()`, `pipeline()`, `parallel()`, `phase()`, `log()` |
| OpenCode | None | No equivalent orchestration runtime |
| Cursor | Partial | Multi-agent parallel execution via /multitask, but no JS orchestration runtime |
| aider | None | No multi-agent orchestration |
