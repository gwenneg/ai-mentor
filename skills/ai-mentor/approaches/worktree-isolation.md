...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | `--worktree` flag, `EnterWorktree` tool, `.worktreeinclude`, subagent isolation |
| OpenCode | Partial | Can work in manually created git worktrees, no automated lifecycle |
| Cursor | Native | Native worktree support, /worktree command, Agents Window for parallel agents |
| aider | None | No built-in worktree support; manual setup possible |
