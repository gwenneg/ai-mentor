...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | `/deep-research <question>`, fan-out search, adversarial verification, cited output |
| OpenCode | None | No built-in research harness; manual web search only |
| Cursor | Partial | `@web` fetches search results but no multi-source verification or adversarial checking |
| aider | None | No web search capabilities |
