...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | Built-in Agent tool, agent teams, Agent View, typed subagents |
| OpenCode | Partial | Primary/subagent system with @mention delegation between instances |
| Cursor | Native | Built-in subagents, custom agent definitions, /multitask for parallel dispatch |
| aider | None | No subagent support; single-session only |
