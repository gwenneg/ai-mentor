...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | `/code-review`, `/security-review`, `/simplify` — built-in skills with --fix and --comment flags |
| OpenCode | None | No equivalent built-in review commands |
| Cursor | Partial | Agent-based review via agent mode and background agents |
| aider | None | No built-in review commands |
