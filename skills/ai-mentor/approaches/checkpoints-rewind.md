...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | Automatic checkpoints on every edit, `/rewind` command (alias `/undo`), Esc Esc shortcut, conversation forking |
| OpenCode | Partial | Git-based undo of file changes, but no integrated checkpoint-conversation separation |
| Cursor | Partial | File-level undo and timeline view, but not tied to conversation state |
| aider | Partial | Auto-commits each change for git-based undo, but no conversation-level checkpoint forking |
