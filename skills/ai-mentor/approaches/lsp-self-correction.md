...
1. Open your project in OpenCode. First, enable LSP by setting `"lsp": true` in your `.opencode.json` (it is disabled by default). Once enabled, OpenCode detects your project's languages and starts the appropriate LSP servers (e.g., `gopls` for Go, `typescript-language-server` for TypeScript).
...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | Official LSP support since v2.0.74, plugin marketplace for 25+ languages, automatic diagnostic feedback loop |
| OpenCode | Native | Auto-detects and starts LSP servers per language, feeds diagnostics into the agent loop automatically |
| Cursor | Partial | Feeds LSP diagnostics via Shadow Workspace and lint iteration; uneven language coverage |
| aider | None (LSP) | Uses tree-sitter linting and auto-fix loop for error correction |
...
You are adding a new endpoint to a Go API service. The endpoint needs to accept a request body, validate it, call two internal services, and return a combined response. You ask OpenCode:
