...
| Tool | Support | Notes |
|------|---------|-------|
| Claude Code | Native | `--permission-mode plan`, Shift+Tab toggle, first-class workflow |
| OpenCode | Native | Built-in Plan Mode via Tab key toggle |
| Cursor | Native | Full Plan Mode as one of four Chat modes since Cursor 2.0 |
| aider | Native | Architect mode is a core feature with dedicated two-model architecture |
