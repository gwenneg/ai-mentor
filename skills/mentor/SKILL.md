...
The **ignorance map** is what remains, and it spans the whole registry: every record — techniques (`registry/techniques.md`), built-in commands (`registry/builtin-commands.md`), integrations and docs (`registry/integrations.md`) — with no `adopted`/`shown`/`declined` row and no positive signal is teachable by default. The one exception is the plugin kind (marketplace plugins in `references/official-plugins.md`, the registry's plugin slice): those enter only when their stack or goal matches the observed work — never as generic filler. Rank by leverage for the work you observed. Signals for the reconcile step come from each record's own `session_signal` line (built-in, integration, and doc records) or, for techniques, from the `adoption-signals.md` table; profile rows use the record's `id` — approach basename, command id, integration id, or plugin name — same table, same statuses. Dedupe the map by `id`: a capability that appears under more than one kind with the same id (e.g. `deep-research`, both the technique and the `/deep-research` built-in command) is one entry and one profile row — teaching or detecting it in either form marks the single id known.
