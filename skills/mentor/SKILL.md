...
The **ignorance map** is what remains: every catalog approach with no `adopted`/`shown`/`declined` row and no positive signal is teachable by default — including promoted marketplace plugins, which are ordinary solutions (`kind: plugin` records in `approaches/`). Unpromoted plugins live in the marketplace directory (`marketplace.md`) with its own entry rule: a directory plugin enters the map only when its stack or goal matches the observed work — never as generic filler. **Don't enumerate the catalog beyond what the mode needs** — and when the user asks a direct inventory question, the full index list IS what the mode needs: this rule limits self-initiated enumeration, never the answer to "what do you know?". Problem mode's candidate set is otherwise just the matched goal file's ranked solutions — techniques, promoted plugins, integrations, every kind competes in the same table; growth mode ranks the whole `approaches/index.md` only if it reaches the lesson opener. Rank by leverage for the work you observed. The index is one row per solution — one approach file, one row, one profile id — and its signal columns are the single signal source for every kind. Profile rows use that `id` (or a plugin name from `marketplace.md`) — same table, same statuses.
...
- End every response with one machine-readable trailer as its own final line, after the closing line: `<!-- mentor mode=problem goal=<goal id|none|browse|decline> move=<approach id|plugin name|own-knowledge> surprise=<approach id|omitted> plugins=<name:tier,...|none> -->` (in growth mode: `mode=growth opener=<followup|transfer|whats-new|lesson|empty> taught=<approach id|none>` instead of goal/move/surprise). Use exact catalog ids. Promoted plugins never appear in `plugins=` (they are first-class approaches); list only directory plugins actually mentioned, as `name:☑️` or `name:⚠️`, or `none`. Inventory answers use `goal=browse`; declines and no-goal answers use `goal=none`. The comment renders invisibly and is not prose — the closing line remains the last visible text — and it must never name a capability the profile marks `declined`
