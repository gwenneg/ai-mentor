...
The **ignorance map** is what remains: every catalog solution with no `adopted`/`shown`/`declined` row and no positive signal is teachable by default — including promoted marketplace plugins, which are ordinary solutions (`kind: plugin` records in `solutions/`). Unpromoted plugins live in the marketplace directory (`marketplace.md`) with its own entry rule: a directory plugin enters the map only when its stack or goal matches the observed work — never as generic filler. **Don't enumerate the catalog beyond what the mode needs:** problem mode's candidate set is just the matched goal file (its ranked approaches plus the `**Built-ins:**`, `**Integrations:**`, and `**Plugins:**` lines); growth mode ranks the whole `solutions/index.md` only if it reaches the lesson opener. Rank by leverage for the work you observed. The index is one row per solution — one `solutions/<id>.md` file, one row, one profile id — and its signal columns are the single signal source for every kind. Profile rows use that `id` (or a plugin name from `marketplace.md`) — same table, same statuses.
...
- Never Read `marketplace.md` whole (~14k tokens): Grep it by the named technology so only matching rows enter context — the goal file's `**Plugins:**` line already carries each goal's top fits
