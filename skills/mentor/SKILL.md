...
- Touch the catalog, the profile, and `~/.claude` paths only with the Read/Glob/Grep tools — never Bash (`ls`, `cat`, `find`, ...): only those tools are guaranteed prompt-free on these paths; Bash prompts for anything beyond a narrow built-in read-only set
