...
2. **Fast path:** if the problem is about a different repo than the one you're in (or no repo at all — a pure tooling question), skip the setup-signal scan entirely: the profile plus the conversation is your evidence, and grounding rules require a portable answer anyway. Otherwise, **read `references/adoption-signals.md`** from the plugin, then check setup signals. Harvest from context first — the loaded CLAUDE.md, the available skills and plugins, and the connected MCP tools are already visible without a single tool call. Then fill gaps with the Read/Glob/Grep tools only (never Bash — read-only tools inside the project and the pre-allowed paths are guaranteed prompt-free; Bash is not): project `.claude/`, `.mcp.json`, CI workflows, and the user-level `~/.claude/settings.json`, `~/.claude/agents/`, `~/.claude/skills/`. Keep it under ~6 checks.
...
The **ignorance map** is what remains: every catalog capability with no `adopted`/`shown`/`declined` row and no positive signal is teachable by default. The one exception is the plugin kind (marketplace plugins in `references/official-plugins.md`, the registry's plugin slice): those enter only when their stack or goal matches the observed work — never as generic filler. **Don't enumerate the catalog here — load only the slice the chosen mode needs, when it needs it:** problem mode's candidate set is just the matched goal file (its ranked approaches plus the `**Built-ins:**` and `**Plugins:**` lines); growth mode reads the full registry (`registry/techniques.md`, `registry/builtin-commands.md`, `registry/integrations.md`) only if it reaches the lesson opener. Rank by leverage for the work you observed, and dedupe by `id`: a capability that appears under more than one kind with the same id (e.g. `deep-research`, both the technique and the `/deep-research` built-in command) is one entry and one profile row. Profile rows use the record's `id` — approach basename, command id, integration id, or plugin name — same table, same statuses. Signals for the reconcile step come from the `adoption-signals.md` table (techniques) or a record's own `session_signal` line (built-in, integration, and doc records, read with the slice above).
