...
Record fields: `id` (profile row key), `kind`, `goals` (slugs from the classification table), `best_when`, `composes_with` (approach basenames or registry ids), `exemplar` (copy-ready line), `session_signal` (what marks it adopted from conversation evidence), `pitfalls`, `source`.
