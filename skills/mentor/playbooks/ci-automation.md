...
| 6 | [claude-code-action](../solutions/claude-code-action.md) | A repo wants Claude reviews or automation on every PR | The maintained GitHub Action packages checkout, auth, and prompt execution — no hand-rolled workflow YAML to maintain |
| 7 | [gitlab-ci-integration](../solutions/gitlab-ci-integration.md) | The team is on GitLab and wants Claude in the pipeline | GitLab's maintained job template is the supported path — one .gitlab-ci.yml job, no GitHub-specific tooling |
