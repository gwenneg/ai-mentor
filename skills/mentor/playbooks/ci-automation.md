...
| 7 | [claude-code-security-review](../approaches/tools/claude-code-security-review.md) | PRs need an automated security pass, not general review | Purpose-built for security: the same diff-aware analysis as `/security-review`, posted as inline comments — narrower but deeper than a general review prompt on claude-code-action |
| 8 | [gitlab-ci-integration](../approaches/tools/gitlab-ci-integration.md) | The team is on GitLab and wants Claude in the pipeline | GitLab's maintained job template is the supported path — one .gitlab-ci.yml job, no GitHub-specific tooling |
