...
| 5 | [agent-sdk-dev](../approaches/tools/agent-sdk-dev.md) | Starting an Agent SDK project from a validated scaffold | A strict-TS scaffold with streaming already wired answers the setup questions so effort goes into the agent's judgment |
| 6 | [agent-sdk](../approaches/tools/agent-sdk.md) | The agent is a product, not a Claude Code workflow | Programmatic sessions and custom tool definitions are a different altitude than .claude/agents/ files — the SDK is the supported path for standalone agents |
