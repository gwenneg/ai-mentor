...
- **Resume, subtask, nest**: Subagents are resumable — ask Claude to resume one and it retains its full conversation history, picking up where it stopped (the built-in Explore and Plan types are one-shot and can't be). `/subtask` inherits the entire parent conversation instead of starting fresh — the tool for tasks that need everything, not a summary. The same inheritance is available directly as an agent type since v2.1.232: spawn one with `subagent_type: "fork"` and it inherits the full conversation and prompt cache. (This in-session behavior used to be `/fork`; since v2.1.207, `/fork` instead copies the conversation into a *new background session* with its own row in agent view — see Background Agents.) And subagents can spawn their own subagents — nested spawning up to depth 3 by default since v2.1.219, tunable with `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` (set it to 1 to disable nesting) — so a reviewer can dispatch a verifier per finding without the intermediate output ever reaching your session.
