...
- **Agent Teams** (experimental — requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` to be enabled): Multiple Claude instances share a task list and message each other directly via `SendMessage`. Unlike simple subagents (which report to a parent), team members are peers: tasks can declare dependencies, and teammates self-claim the next unblocked task when they finish one. Best for work requiring discussion — competing debugging hypotheses, multi-lens reviews — at a significantly higher token cost than subagents. Separately, and without any experiment flag, full Claude Code sessions can message each other since v2.1.224: cross-session `SendMessage` reaches your sessions on any of your machines, with `ListAgents` to discover them — macOS, Linux, and (since v2.1.239) native Windows. Type `@` in the prompt to mention another session by name and Claude messages it directly with `SendMessage` (v2.1.232+); the `notify_when_idle` input on `SendMessage` asks a session to send one notice when it next goes idle (v2.1.234+).
...
- **Resume, subtask, nest**: Subagents are resumable — ask Claude to resume one and it retains its full conversation history, picking up where it stopped (the built-in Explore and Plan types are one-shot and can't be). `/subtask` inherits the entire parent conversation instead of starting fresh — the tool for tasks that need everything, not a summary. (This in-session behavior used to be `/fork`; since v2.1.207, `/fork` instead copies the conversation into a *new background session* with its own row in agent view — see Background Agents.) And subagents can spawn their own subagents — nested spawning up to depth 3 by default since v2.1.219, tunable with `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` (set it to 1 to disable nesting) — so a reviewer can dispatch a verifier per finding without the intermediate output ever reaching your session.
- **Fork mode by default**: Since v2.1.232, fork mode is on by default in interactive sessions — Claude can request the `fork` subagent type, which inherits the full conversation and prompt cache instead of starting fresh, so a side task does not need re-explaining. Set `CLAUDE_CODE_FORK_SUBAGENT=0` to turn it off.
