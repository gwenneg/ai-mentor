...
- **Design iteration on mockups**: For greenfield UI work, generate two or three visual directions as separate artifacts and A/B them with stakeholders before writing any application code. The `/design` skill (research preview, v2.1.234+) packages this pattern: `/design redesign the composer based on what people actually use it for` publishes a canvas of editable artboards built on artifacts, and once you pick one, Claude implements it.
