...
- **PR-based worktrees**: `claude --worktree "#1234"` (or a full PR URL) fetches `pull/1234/head` from `origin` and creates the worktree at `.claude/worktrees/pr-1234` — a teammate's PR checked out for review or fixes without touching your branch. Since v2.1.233 the same `--worktree` flag (and the `claude agents` view, where they display as `!N`) also accepts GitLab merge request URLs.
