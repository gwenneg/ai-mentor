...
- **PR-based worktrees**: `claude --worktree "#1234"` (or a full PR URL) fetches `pull/1234/head` from `origin` and creates the worktree at `.claude/worktrees/pr-1234` — a teammate's PR checked out for review or fixes without touching your branch. When `origin` is on gitlab.com, pass a merge request URL instead (`claude --worktree https://gitlab.com/group/project/-/merge_requests/42`, v2.1.233+): Claude Code fetches `merge-requests/42/head` and opens the worktree on that branch, with `claude agents` labeling the session `!42`.
