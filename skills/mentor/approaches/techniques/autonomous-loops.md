...
- **Background work inside a goal loop**: When a turn spawns background tasks (subagents, background sessions) that keep `/goal` waiting on their result, Claude checks in on them after 30 minutes rather than blocking indefinitely, then keeps checking in at longer intervals while the session sits idle (v2.1.239+). Set `CLAUDE_CODE_GOAL_CHECKIN_MINUTES=0` to opt out of the check-ins.
