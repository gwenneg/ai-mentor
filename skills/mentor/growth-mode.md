...
4. **The lesson.** Only now, having ruled out openers 1–3, build the full ignorance map: read the compiled index (`solutions/index.md`, one row per capability), subtract the profile and observed signals, and teach the top of what remains (every kind is an equal citizen — a lesson can be "you've never used /verify" or "pr-review-toolkit exists" just as well as a technique; a marketplace-directory plugin qualifies only when its stack/goal relevance to this user is concrete). Before teaching, read the winner's own `solutions/<id>.md` for the deep-dive. Hook the lesson to their observed work ("you do X by hand; this removes that"), name the principle in one sentence, offer the live demo, set it up on acceptance. One capability. Not two.
