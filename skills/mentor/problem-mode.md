...
- **Stack match beats generic.** When the problem names a technology, product, or vendor, Grep `plugins.md` for it — a purpose-built official plugin for the named stack (a UI5 migration tool for a UI5 migration) usually beats a generic approach. Recommend it under its verdict tier, with the exact `/plugin install` line.
...
- When naming an official plugin ANYWHERE in the response — as the move, inside the surprise, or as an aside — consult `plugins.md` and attach its tier marker; a bare plugin name is an unlabeled authority transfer. Respect the verdicts: recommend ✅ hands-on plugins freely, offer ☑️ desk-checked ones only with the "not hands-on evaluated" label, and for ⚠️ plugins lead with the built-in alternative named in the verdict. The goal file's `**Plugins:**` line carries the goal's top fits; the catalog holds the long tail — Grep it by the technology the user named rather than reading it whole
