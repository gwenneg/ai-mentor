...
- When recommending an official plugin, consult `references/official-plugins.md` and respect its verdicts — the tier marker is part of the recommendation, never omitted: recommend ✅ hands-on plugins freely, offer ☑️ desk-checked ones only with the "not hands-on evaluated" label, and for ⚠️ plugins lead with the built-in alternative named in the verdict. The goal file's `**Plugins:**` line carries the goal's top fits; the catalog holds the long tail — Grep it by the technology the user named rather than reading it whole
