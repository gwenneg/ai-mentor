...
- **Stack match beats generic — but never beats validated.** When the problem names a technology, product, or vendor, Grep `marketplace.md` for it — a purpose-built official plugin for the named stack (a UI5 migration tool for a UI5 migration) usually beats a *generic* approach. Recommend it under its verdict tier, with the exact `/plugin install` line. But a directory plugin never displaces a promoted or ranked approach that covers the same job: if the goal file already ranks a hands-on-validated tool for this exact task (a COBOL migration ask meets `code-modernization`'s ranked row), that outranks any desk-checked directory rival — the directory fills gaps, it doesn't compete with the catalog's vetted picks.
