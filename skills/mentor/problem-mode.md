...
Read the matched goal's file at `problems/<goal>.md` (one small file per goal). How to read it: the ranked rows, hidden gem, and plugin picks are editorial judgment — they change when we change our minds; verifiable product claims live in the approach files. The **Exemplar move** is a fictional prompt showing the *shape* of a well-grounded move — never show it verbatim: rewrite it against the actual repo, or keep it portable when the problem targets another repo.

Choose **the move**: the goal file's #1 ranked approach, unless the evidence points elsewhere — or unless the profile says they already use it, in which case build on it and take the next-best they don't know. Promoted plugins are ranked rows like any approach — a plugin move needs no special rule; read its `solutions/<id>.md` record for the install line, facts, and pitfalls. **The move can be a built-in command or an integration**: the goal file's `**Built-ins:**` and `**Integrations:**` lines name the record solutions that fit this goal — when one removes the problem more directly than any approach (the diff needs `/code-review` more than it needs a methodology; "Claude reviewing every PR" IS `claude-code-action`), recommend it as the move: read its `solutions/<id>.md` record for the exemplar or facts, install line, and pitfalls, give the copy-ready command or the concrete setup step, and record the profile row under its `id`. **The move can also be a directory plugin** — surfaced only by the stack-match grep of `marketplace.md` (directory plugins are never listed in goal files): when one matches the observed stack or removes the problem more directly than the top approach, recommend it as the move (verdict rules apply), with the `/plugin install` line and what it unlocks; otherwise a fitting plugin still belongs in the recommendation as the tool the chosen approach wields. The "why it fits" line is curated judgment — use it to frame the pitch, then ground the substance in the approach file and the repo. Choose **the surprise**: the highest-leverage entry from their ignorance map that's relevant to this goal — and in problem mode that map is just the goal file's own candidate set (its ranked solutions plus the `**Built-ins:**` and `**Integrations:**` lines) minus the profile and observed signals, so no full-registry read is needed. Every solution kind is an equal citizen here (technique, built-in, integration, doc, or promoted plugin), and a stack-relevant directory plugin qualifies too; fall back to the goal file's `**Hidden gem:**` line only when the profile is empty. The surprise is default-on — it's the reason this plugin exists — but it has a relevance floor, and under `incident-response` classification with a narrow question the floor is binding: omit the surprise, full stop — the discovery moment waits for calmer water. Outside incidents, omit it when nothing in the ignorance map genuinely relates to this goal or stack rather than pad with filler. An irrelevant discovery moment costs more trust than a missing one.
...
- When naming a marketplace-directory plugin ANYWHERE in the response — as the move, inside the surprise, or as an aside — consult `marketplace.md` and attach its tier marker; a bare plugin name is an unlabeled authority transfer. Respect the verdicts: offer ☑️ desk-checked ones only with the "not hands-on evaluated" label, and for ⚠️ plugins lead with the built-in alternative named in the verdict. Promoted plugins (those with a `solutions/<id>.md` record, ranked in goal files like any approach) need no directory lookup — they are hands-on evaluated by definition; their record carries the facts and pitfalls. The directory holds the long tail — Grep it by the technology the user named rather than reading it whole
