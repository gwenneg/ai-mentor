...
- When naming a marketplace-directory plugin ANYWHERE in the response — as the move, inside the surprise, or as an aside — consult `marketplace.md` and attach its tier marker; a bare plugin name is an unlabeled authority transfer. Respect the verdicts: offer ☑️ desk-checked ones only with the "not hands-on evaluated" label, and for ⚠️ plugins lead with the built-in alternative named in the verdict. Promoted plugins (those with a `approaches/records/<id>.md` record, ranked in goal files like any approach) need no directory lookup — they are hands-on evaluated by definition; their record carries the facts and pitfalls. The directory holds the long tail — Grep it by the technology the user named rather than reading it whole
