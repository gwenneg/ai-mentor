...
*Last synced: 2026-08-14 · Source: [`anthropics/claude-plugins-official`](https://github.com/anthropics/claude-plugins-official) marketplace manifest*
...
| `claude-security` | Deep in-session vulnerability scanning at a chosen effort tier, with every finding challenged before being reported and surviving findings turned into agent-verified patches | `security` | ⚠️ overlaps the built-in `/security-review` skill and the promoted `security-guidance` plugin — recommend the built-in first; desk-checked 2026-07-27 |
...
| `synthflow` | Connects Claude Code to the Synthflow AI voice-agent platform through its hosted MCP server, with skills for reviewing calls and auditing agent prompts, plus a docs-search connector | Synthflow | `building-agents` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs a Synthflow account |
...
| Plugin | What it does | Author | Relevant goal | Verdict |
|--------|-------------|--------|--------------|---------|
| `aiven` | Deploy managed PostgreSQL, Kafka, OpenSearch, ClickHouse and other databases, streaming, and apps; free tier available | Aiven | `devops` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs an Aiven account |
...
| `azure-sql-developer` | Agent skills for the Azure SQL Developer local-container engine: run, connect, migrate, scaffold, wire CI, go local-to-cloud | Microsoft | `devops` | ☑️ desk-checked 2026-07-27 — needs the Private Preview Azure SQL Developer container image |
...
| `mongodb-atlas` | Connect to MongoDB Atlas clusters via the Atlas Managed MCP Server: explore data, manage collections, optimize queries, manage clusters/projects/users/network access | MongoDB | `devops` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs a MongoDB Atlas account (self-managed deployments should use the `mongodb` plugin instead) |
...
| `google-cloud-storage` | Manage GCS buckets and objects, transfer data, and configure MCP, FUSE, IAM, security, lifecycle rules, signed URLs, Terraform, and the CLI | Google LLC | `devops` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs a Google Cloud project |
...
| `superdesign` | Design or redesign frontend UI and marketing graphics on the Superdesign infinite canvas; reads the codebase for context, sets up a design system, and generates branchable design drafts | Superdesign dev, Inc. | `greenfield` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised |
...
| `airwallex-dev` | Generates checkout, card-element, onboarding, and subscription-billing code for Airwallex payment integrations: Hosted Payment Page, Drop-in/Split Card elements, KYC onboarding, Billing Hosted Checkout | Airwallex | `greenfield` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs an Airwallex account |
...
| `catalyst-by-zoho` | Official plugin for Catalyst by Zoho's full-stack serverless cloud platform: services, SDKs, CLI, architecture patterns, migration guides | Catalyst by Zoho | `greenfield` | ☑️ desk-checked 2026-07-27 — needs a Catalyst by Zoho account |
...
| `ckeditor` | Install, configure, and integrate CKEditor 5 (free and premium) in any JavaScript project | CKEditor (CKSource) | `greenfield` | ☑️ desk-checked 2026-07-27 — needs a CKEditor license for premium features |
...
| `gitkraken` | Git and project context across every repo: commits, branches, PRs, and issues across GitHub, GitLab, Azure DevOps, Bitbucket, and Jira | GitKraken | `code-review` | ☑️ desk-checked 2026-07-27 — needs GitKraken auth to the relevant provider(s) |
...
| `modern-web-guidance` | Keeps the coding agent current on modern web platform best practices | Google Chrome | `greenfield` | ☑️ desk-checked 2026-07-27 — reference guidance, no external account needed |
...
| `qodo` | Qodo skills for code quality checks, testing, security scanning, compliance validation | unlabeled | `code-review` | ☑️ desk-checked 2026-07-27 — renamed from `qodo-skills`; same source repo (qodo-ai/qodo-skills), component inventory not re-counted since the rename; needs Qodo platform account |
...
| `salesforce-development` | Build Salesforce apps and agents: metadata, Apex, deploy/retrieve, security, reporting, and installed-vs-available capability discovery | Salesforce | `greenfield` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs a Salesforce org |
...
| `streaming-skills-plugin` | Skills for streaming application developers covering Kafka and Flink client libraries and Schema Registry | Confluent | `devops` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised |
...
| `noibu` | Connects Claude to ecommerce session, error, and conversion data through Noibu, plus the marketing, support, and commerce platforms that put insight into motion | Noibu | `incident-response` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs a Noibu account |
...
| `atlassian-twg-cli` | Atlassian's agent-first CLI interface to Jira issues, Confluence pages, Bitbucket PRs, and connected third-party data via the Teamwork Graph | Atlassian | `documentation` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs an Atlassian account and `/twg-setup` after install |
...
| `crowdstrike-falcon-fusion` | CrowdStrike Falcon Fusion skills for authoring, deploying, and executing workflows: live action discovery, YAML authoring with schema validation, import/release, execution monitoring, SIEM lookup files | CrowdStrike | `security` | ☑️ desk-checked 2026-08-14 — added via catalog sync, not yet exercised; needs CrowdStrike Falcon Fusion access |
...
| `stackhawk-api` | Query the StackHawk platform API for security posture reporting, findings analysis, and app management | StackHawk | `security` | ☑️ desk-checked 2026-07-27 — needs a StackHawk account |
| `stackhawk-hawkscan` | Configure, run, and interpret HawkScan DAST scans; generates `stackhawk.yml` and prioritized fix tasks | StackHawk | `security` | ☑️ desk-checked 2026-07-27 — needs a StackHawk account; shares a source repo with `stackhawk-api` (different subdirectory) |
...
| Plugin | What it does | Author | Relevant goal | Verdict |
|--------|-------------|--------|--------------|---------|
| `growthbook` | Full lifecycle skills for GrowthBook feature flags and experimentation | GrowthBook | `release-management` | ☑️ desk-checked 2026-07-27 — needs a GrowthBook account |
