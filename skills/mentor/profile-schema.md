...
Last new-capability check: <what's-new week slug, e.g. 2026-w26 — always a week slug; on profile creation set it to the current ISO week (first meeting is the baseline), never "never" — derived from today's date in context, never by running `date`, which prompts>
