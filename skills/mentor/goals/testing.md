...
When generating test code, type errors are the most common waste of time — you write a test, run it, wait 30 seconds, and discover you passed a `string` where the function expects a `number`. With an LSP plugin installed (e.g. `typescript-lsp` from the official marketplace), diagnostics feed straight into Claude's editing loop, fixing type mismatches, missing imports, and incorrect method signatures before you ever hit "run." Fewer wasted test cycles means faster coverage gains.
...
**Cons:**
- Requires the language's LSP plugin and language server to be installed
