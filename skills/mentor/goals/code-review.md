...
### 6. Official Plugins — Ready-made review workflows out of the box
**Level:** Intermediate
...
### 7. Cloud Sessions — Deep review off your machine, and PRs that fix themselves
**Level:** Intermediate

Two cloud moves change review economics. `/code-review ultra` runs a deep multi-agent review in a cloud sandbox while your terminal stays free. And `/autofix-pr` on a PR branch spawns a cloud session that watches the PR — when CI fails or a reviewer comments, it investigates and pushes the fix if one is clear, asking you first when the request is ambiguous.

**Try it now:**
> I've opened the PR for this branch. Run /autofix-pr so a cloud session watches it — it should handle CI failures and mechanical review comments, and ask me before acting on anything architectural.

**Why this works:** Most post-review churn is mechanical — a lint failure, a rename request, a missing null check. Delegating the churn to a watching agent keeps human reviewers focused on design and keeps the PR moving between your check-ins.

**Pros:**
- Review depth without occupying your machine or your attention
- Auto-fix responds to PR events within minutes, around the clock
- Escalates ambiguous or architectural requests instead of guessing

**Cons:**
- Replies post under your GitHub account (labeled as Claude Code) — mind comment-triggered automation
- Can't resolve merge conflicts on its own (GitHub sends no webhook for them)

**Deeper:** See `approaches/cloud-sessions.md`
