...
### 6. Permissions & Safe Autonomy — Hard boundaries around the infrastructure that matters
**Level:** Intermediate

Infrastructure work is where an AI mistake costs the most, and where permission rules earn their keep: deny rules are enforced by Claude Code regardless of what the model decides. Deny `terraform apply` and `kubectl delete` outright, deny edits under `infra/production/`, allowlist the read-only planning commands — and iteration on infrastructure code becomes safe enough to let the agent run freely on everything else.

**Try it now:**
> Add to .claude/settings.json: deny `Bash(terraform apply *)`, `Bash(kubectl delete *)`, and `Edit(/infra/production/**)`; allow `Bash(terraform plan *)`, `Bash(terraform validate *)`, and `Bash(helm template *)`. Then iterate on the staging module — plan output is our feedback loop, and apply stays human-only.

**Why this works:** Guardrails beat vigilance: a deny rule can't have a tired Friday afternoon, and unlike CLAUDE.md guidance it binds no matter what the model attempts.

**Pros:**
- Plan/template/validate loops run friction-free while apply stays gated
- Checked-in project denies become team-wide invariants (deny wins across all settings layers)
- Pairs with sandboxing for OS-level enforcement on top

**Cons:**
- Bash patterns constraining *arguments* are fragile — prefer denying whole commands and gating narrow allows
- Rules need updating as your infra tooling evolves
